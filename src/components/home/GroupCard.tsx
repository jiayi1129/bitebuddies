...
export default function GroupCard({ group, userId }: {group: Document<"groups">, userId: Id<"users"> | null}) {
  ...
  const formattedDate = `${dateTime.toLocaleDateString("en-US", { day: "numeric", month: "short" })}, ${dateTime.toLocaleTimeString("en-US", { hour: "numeric", minute: "numeric", hour12: true })}`;
  const bodyText = group.users.length + "/" + group.capacity + " people attending";
  return (
